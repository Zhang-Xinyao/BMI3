...
# def rabin_karp_search_with_mismatches(pattern, text, max_mismatches=5, d=4, q=101):
#     n = len(text)
#     m = len(pattern)
#     h = pow(d, m - 1) % q
#     p = 0
...
#         if p == t:
#             mismatches = sum(1 for a, b in zip(pattern, text[s:s + m]) if a != b)
#             if mismatches <= max_mismatches:
#                 results.append((s, mismatches))
#         if s < n - m:
...
# import re
#
# def boyer_moore_search(grna, target_sequence, pam = re.compile(r'N?GG', re.IGNORECASE), max_mismatches=5):
#     """
#     Boyer-Moore 搜索：评估 gRNA 在目标序列中的结合效率
#     Args:
#         grna: gRNA 序列
#         target_sequence: 目标序列（基因区域序列）
#         pam: PAM 序列（默认 NGG）
#         max_mismatches: 最大允许错配数
#     Returns:
#         matches: 匹配位点列表，包含位点位置和评分
#     """
#     matches = []
#     grna_len = len(grna)
#     pam_len = 3
...
#     bad_char = {char: grna.rfind(char) for char in set(grna)}  # 创建坏字符表
#
#     i = 0
#     while i <= len(target_sequence) - grna_len - pam_len:
#         # 滑窗的 gRNA 部分和 PAM 部分
#         window_grna = target_sequence[i:i + grna_len]
#         downstream_pam = target_sequence[i + grna_len:i + grna_len + pam_len]
#
#         # 检查错配数量
#         mismatches = sum(1 for a, b in zip(grna, window_grna) if a != b)
#
#         # 检查 PAM 序列并计算评分
#         if mismatches <= max_mismatches and downstream_pam == pam:
#             score = 1.0 - 0.1 * mismatches  # 根据错配数量打分
#             matches.append((i, score))
#
#         # 使用坏字符规则决定滑动多少位置
#         if i + grna_len < len(target_sequence) and target_sequence[i + grna_len] in bad_char:
#             i += max(1, grna_len - bad_char[target_sequence[i + grna_len]])
#         else:
#             i += 1
#
#     return matches
#
# find=0
# for transcript_id, sequences in pos_sequences.items():
#         for seq in sequences:
#             if len(seq) >= len(gRNA):
#                 find+=len(boyer_moore_search(gRNA,seq,pam=re.compile(r'N?GG', re.IGNORECASE),max_mismatches=5))
# print(find)
...
gRNA_off_target = dict(find_off_target(pos_sequences,gRNA,max_mismatch))
data=[]
...
# related off-target information
# df = df.groupby("gRNA").agg(
#     Total_Mismatches=("mismatch", "sum"),
#     Average_Mismatch=("mismatch", "mean"),
#     Sequence_Count=("DNA", "count")  # 统计gRNA有几个结合位点
# ).reset_index()
# print(df)
sg_ls=df["gRNA"]
tg_ls=df["DNA"]
...
result=MOFF_score(m1_dic, m2_dic, df)
